...
def random_location():
    cities = ["New York", "Los Angeles", "London", "Paris", "Berlin",
              "Tokyo", "Sydney", "Mumbai", "Dubai", "Rio de Janeiro"]
...
def populate_eventcalendar_table(events=100):
    ...
    for _ in range(events):
        ...
        # Event date is after album creation
        eventDate = dateCreated + timedelta(days=random.randint(1, 365))
        eventTime = random_time()
        location = random_location()
        revenueGenerated = round(random.uniform(
            1000, 100000), 2)  # Random revenue
